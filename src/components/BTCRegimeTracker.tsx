...
}: {indicator: Indicator;status: Status;onStatusChange: (status: Status) => void;expanded: boolean;onToggle: () => void;}) => {
...
}: {phase: Phase;phaseKey: string;indicators: Record<string, Status>;expanded: boolean;onToggle: () => void;onIndicatorStatusChange: (id: string, status: Status) => void;expandedIndicator: string | null;onIndicatorToggle: (id: string) => void;}) => {
      ...
      <div className="sticky top-0 z-10 bg-slate-950/90 backdrop-blur-lg border-b border-slate-800/50">
        <div className="px-4 py-3">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="font-bold text-slate-100 tracking-tight text-3xl text-center">CB's BTC Bull/Bear Regime Checklist</h1>
              <p className="text-slate-500 py-[10px] text-lg font-sans font-bold">When is it cheap enough to buy Bitcoin?</p>
            </div>
            <button
              onClick={resetAll}
